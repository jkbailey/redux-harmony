export const createAsyncAction = (namespace, promise) => {
  ...
  const loadingFunc = () => ({ type: constants.LOADING, status: {loading: true, error: false} })
  const successFunc = data => ({ type: constants.SUCCESS, data, status: {loading: false, error: false} })
  const errorFunc = error => ({ type: constants.ERROR, error, status: {loading: false, error: true} })
